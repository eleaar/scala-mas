...
object Genetic {
  ...
  def createSolution(implicit config: EmasConfig): Solution =
  ...
  def reproduce(s: Solution)(implicit config: EmasConfig) = mutateSolution(s)

  def reproduce(s1: Solution, s2: Solution)(implicit config: EmasConfig) =
    mutateSolutions(recombineSolutions(s1, s2))

  def mutateSolution(s: Solution)(implicit config: EmasConfig) =
    if (random < config.mutationChance)
      s.map(f => if (random < config.mutationRate) mutateFeature(f) else f)
    else
      s

  def mutateSolutions(s: (Solution, Solution))(implicit config: EmasConfig) =
    (mutateSolution(s._1), mutateSolution(s._2))

  def mutateFeature(f: Feature)(implicit config: EmasConfig): Feature = {
